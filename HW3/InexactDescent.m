%Inexact Descent Algortihm using different search methods for line search
%%% Inputs:
% func: The function to be minimized
% initial: Initial guess for the input vector
% search_condition: Which line search conditions are used:
% 'Armijo' : Use first Armijo condition for inexact line search
% 'Armijo_Goldstein' : Use Armijo-Goldstein conditions for line search
...
% iteration: Number of iterations for vector calculations
...
function [final_vector,final_value,iteration,search_iter]=InexactDescent(func,initial,search_condition,end_condition,bound)
    x=initial;
    ...
    if end_condition==1 % First stop condition !
        while (normtwo(x-x_last)>=bound) % Stop iterating when the norm of difference falls below the bound.
            ...
            if normtwo(g)~=0
            [alpha,iter_search]=search_condition(func,x_last,g); % ExactLineSearch with the use of secant method. 
            end
            x=x_last-alpha*g; % Use gradient algortihm to calculate the next point
    ...
    elseif end_condition==2 % Second stop condition !
        ...
        while (normtwo(g)>=bound) % Stop iterating when the norm of the gradient falls below the bound.
            ...
            if normtwo(g)~=0
            [alpha,iter_search]=search_condition(func,x_last,g);
            end
            x=x_last-alpha*g; % Calculate the next point with gradient descent algorithm.
