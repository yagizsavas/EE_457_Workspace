%Steepest Descent Algortihm using Secant method for exact line search
%%% Inputs:
...
function [final_vector,final_value,iteration,search_iter]=SteepestDescent(func,initial,end_condition,bound)
    x=initial;
    ...
    if end_condition==1 % First stop condition !
        while (normtwo(x-x_last)>=bound) % Stop iterating when the norm of difference falls below the bound.
            ...
            if normtwo(g)~=0
            [alpha,iter_search]=ExactLineSearch(func,x_last,1,1.1,10^-4); % ExactLineSearch with the use of secant method. An improvement 
                                                            % on initial conditions of the method may be required.
            end
    ...
    elseif end_condition==2 % Second stop condition !
        ...
        while (normtwo(g)>=bound) % Stop iterating when the norm of the gradient falls below the bound.
            ...
            if normtwo(g)~=0
            [alpha,iter_search]=ExactLineSearch(func,x_last,1,1.1,10^-4);
            end
